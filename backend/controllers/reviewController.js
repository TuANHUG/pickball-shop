...
const addReview = async (req, res) => {
  try {
    ...
    const average =
      quantity > 0
        ? reviews.reduce((acc, curr) => acc + curr.rating, 0) / quantity
        : 0;
  ...
  try {
    ...
    if (!productId || !orderId) {
      return res
        ...
        .json({
          success: false,
          message: "Thiếu thông tin productId hoặc orderId",
        });
    }
...
const updateReview = async (req, res) => {
  try {
    ...
    if (keptImages) {
      try {
        keptImagesIds = JSON.parse(keptImages);
      } catch (e) {
        keptImagesIds = [keptImages];
    ...
    const imagesToDelete = review.images.filter(
      (img) => !keptImagesIds.includes(img.public_id)
    );
    ...
    const imagesToKeep = review.images.filter((img) =>
      keptImagesIds.includes(img.public_id)
    );
    ...
    res
      .status(200)
      .json({ success: true, message: "Cập nhật đánh giá thành công", review });
  } catch (error) {
  ...
  try {
    ...
    if (!reviewId || !comment) {
      return res
        ...
        .json({
          success: false,
          message: "Thiếu thông tin reviewId hoặc nội dung trả lời",
        });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Trả lời đánh giá thành công", review });
  } catch (error) {
...
const deleteReply = async (req, res) => {
  try {
    ...
    if (!reviewId) {
      return res
        .status(400)
        .json({ success: false, message: "Thiếu thông tin reviewId" });
    }
...
const hideReview = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: review.hidden ? "Đã ẩn đánh giá" : "Đã hiện đánh giá",
        review,
      });
  } catch (error) {
